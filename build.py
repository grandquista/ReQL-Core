...
def convert_tests(tests, lang):
    lang_tests = []
    tables = {table: scripting.test_results.TestTable(table) for table in get_tables(tests)}
...
#                lang_test.append('\n'.join([eval_section(s, tables) for s in section]))
            try:
                lang_test.append(scripting.test_results.eval_result(test.get('ot', {'result': 'blank'}), lang))
            except scripting.test_results.BadKeyError:
                lang_test.append(scripting.test_results.eval_result({'result': 'error'}, lang))
            if lang == 'objc':
