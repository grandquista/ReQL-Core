...
TEST_CASE("c Tests that manipulation data in tables", "[c][ast]") {
  ...
  SECTION("test15") {
    ...
    std::unique_ptr<uint8_t[]> buf31(new uint8_t[2]);
    const uint8_t src31[] = "id";
    reql_string_init(var31.get(), buf31.get(), 2);
    reql_string_append(var31.get(), src31, 2);
