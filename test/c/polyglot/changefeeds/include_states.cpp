...
TEST_CASE("c Test `include_states`", "[c][ast]") {
  ...
  SECTION("test14") {
    ...
    std::unique_ptr<ReQL_Obj_t> var22(new ReQL_Obj_t);
    std::unique_ptr<uint8_t> buf22(new uint8_t[7]);
    const uint8_t src22[] = "new_val";
    reql_string_init(var22.get(), buf22.get(), 7);
    reql_string_append(var22.get(), src22, 7);
