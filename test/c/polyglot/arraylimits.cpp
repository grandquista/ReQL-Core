...
TEST_CASE("c Tests array limit variations", "[c][ast]") {
  ...
  SECTION("test12") {
    ...
    const uint8_t src3[] = "array";
    reql_string_init(var3.get(), buf3.get(), 5);
    reql_string_append(var3.get(), src3, 5);

    std::unique_ptr<ReQL_Obj_t> var4(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Obj_t*[]> arr4(new ReQL_Obj_t*[10]);
    reql_array_init(var4.get(), arr4.get(), 10);
    ...
    reql_array_append(var4.get(), var7.get());
    ...
    std::unique_ptr<ReQL_Obj_t> var9(new ReQL_Obj_t);
    reql_number_init(var9.get(), 5);

    reql_array_append(var4.get(), var9.get());

    std::unique_ptr<ReQL_Obj_t> var10(new ReQL_Obj_t);
    reql_number_init(var10.get(), 6);

    reql_array_append(var4.get(), var10.get());

    std::unique_ptr<ReQL_Obj_t> var11(new ReQL_Obj_t);
    reql_number_init(var11.get(), 7);
