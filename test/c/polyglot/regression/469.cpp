...
TEST_CASE("c Issue", "[c][ast]") {
  ...
  SECTION("test11") {
    ...
    const uint8_t src7[] = "primary_key";
    ...
    const uint8_t src24[] = "indexes";
    reql_string_init(var24.get(), buf24.get(), 7);
    reql_string_append(var24.get(), src24, 7);

    std::unique_ptr<ReQL_Obj_t> var25(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Obj_t*> arr25(new ReQL_Obj_t*[1]);
    reql_array_init(var25.get(), arr25.get(), 1);

    std::unique_ptr<ReQL_Obj_t> var26(new ReQL_Obj_t);
    std::unique_ptr<uint8_t> buf26(new uint8_t[1]);
    const uint8_t src26[] = "x";
    reql_string_init(var26.get(), buf26.get(), 1);
    reql_string_append(var26.get(), src26, 1);

    reql_array_append(var25.get(), var26.get());

    reql_object_add(var0.get(), var24.get(), var25.get());
  }
  ...
  SECTION("test12") {
    ...
    const uint8_t src9[] = "primary_key";
    ...
    const uint8_t src16[] = "db";
    reql_string_init(var16.get(), buf16.get(), 2);
    reql_string_append(var16.get(), src16, 2);

    std::unique_ptr<ReQL_Obj_t> var17(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Pair_t> pair17(new ReQL_Pair_t[3]);
    reql_object_init(var17.get(), pair17.get(), 3);
    ...
    std::unique_ptr<ReQL_Obj_t> var22(new ReQL_Obj_t);
    std::unique_ptr<uint8_t> buf22(new uint8_t[4]);
    const uint8_t src22[] = "func";
    reql_string_init(var22.get(), buf22.get(), 4);
    reql_string_append(var22.get(), src22, 4);

    std::unique_ptr<ReQL_Obj_t> var23(new ReQL_Obj_t);
    std::unique_ptr<uint8_t> buf23(new uint8_t[4]);
    const uint8_t src23[] = "uuid";
    reql_string_init(var23.get(), buf23.get(), 4);
    reql_string_append(var23.get(), src23, 4);
