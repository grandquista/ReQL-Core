...
TEST_CASE("c 831 -- Empty batched_replaces_t constructed", "[c][ast]") {
  ...
  SECTION("test0") {
    ...
    std::unique_ptr<uint8_t> buf10(new uint8_t[36]);
    const uint8_t src10[] = "Expected type OBJECT but found BOOL.";
    reql_string_init(var10.get(), buf10.get(), 36);
    reql_string_append(var10.get(), src10, 36);
