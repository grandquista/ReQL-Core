...
TEST_CASE("c Tests replacement of selections", "[c][ast]") {
  ...
  SECTION("test5") {
    ...
    std::unique_ptr<ReQL_Obj_t> var10(new ReQL_Obj_t);
    std::unique_ptr<uint8_t> buf10(new uint8_t[62]);
    const uint8_t src10[] = "Inserted object must have primary key `id`:\n{\n\t\"a\":\t1\n}";
    reql_string_init(var10.get(), buf10.get(), 62);
    reql_string_append(var10.get(), src10, 62);
