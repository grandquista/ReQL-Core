...
TEST_CASE("c Tests RQL control flow structures", "[c][ast]") {
  ...
  SECTION("test55") {
    ...
    std::unique_ptr<ReQL_Obj_t> var10(new ReQL_Obj_t);
    std::unique_ptr<uint8_t> buf10(new uint8_t[83]);
    const uint8_t src10[] = "Duplicate primary key `id`:\n{\n\t\"foo\":\t3,\n\t\"id\":\t1\n}\n{\n\t\"id\":\t1\n}";
    reql_string_init(var10.get(), buf10.get(), 83);
    reql_string_append(var10.get(), src10, 83);
