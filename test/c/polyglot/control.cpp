...
TEST_CASE("c Tests RQL control flow structures", "[c][ast]") {
  ...
  SECTION("test55") {
    ...
    std::unique_ptr<ReQL_Obj_t> var4(new ReQL_Obj_t);
    std::unique_ptr<uint8_t[]> buf4(new uint8_t[83]);
    const uint8_t src4[] = "Duplicate primary key `id`:\n{\n\t\"foo\":\t3,\n\t\"id\":\t1\n}\n{\n\t\"id\":\t1\n}";
    reql_string_init(var4.get(), buf4.get(), 83);
    reql_string_append(var4.get(), src4, 83);
  ...
  SECTION("test57") {
    ...
    std::unique_ptr<ReQL_Obj_t> var7(new ReQL_Obj_t);
    std::unique_ptr<uint8_t[]> buf7(new uint8_t[14]);
    const uint8_t src7[] = "generated_keys";
    reql_string_init(var7.get(), buf7.get(), 14);
    reql_string_append(var7.get(), src7, 14);
    ...
    const uint8_t src9[] = "func";
    reql_string_init(var9.get(), buf9.get(), 4);
    reql_string_append(var9.get(), src9, 4);

    std::unique_ptr<ReQL_Obj_t> var10(new ReQL_Obj_t);
    std::unique_ptr<uint8_t[]> buf10(new uint8_t[6]);
    const uint8_t src10[] = "arrlen";
    reql_string_init(var10.get(), buf10.get(), 6);
    reql_string_append(var10.get(), src10, 6);
