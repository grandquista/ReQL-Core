...
TEST_CASE("reql Tests for the basic usage of the multiplication operation", "[reql][ast]") {
  ...
  SECTION("test2") {
    std::unique_ptr<ReQL_Obj_t> var0(new ReQL_Obj_t);
    reql_number_init(var0.get(), 6.75);
  }

  SECTION("test3") {
    std::unique_ptr<ReQL_Obj_t> var0(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Obj_t*[]> arr0(new ReQL_Obj_t*[9]);
    reql_array_init(var0.get(), arr0.get(), 9);
