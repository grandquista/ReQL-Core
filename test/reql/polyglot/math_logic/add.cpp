...
TEST_CASE("reql Tests for basic usage of the add operation", "[reql][ast]") {
  ...
  SECTION("test3") {
    std::unique_ptr<ReQL_Obj_t> var0(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Byte[]> buf0(new ReQL_Byte[0]);
    const ReQL_Byte src0[] = "";
    reql_string_init(var0.get(), buf0.get(), src0, 0);
  }

  SECTION("test4") {
    std::unique_ptr<ReQL_Obj_t> var0(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Byte[]> buf0(new ReQL_Byte[6]);
    const ReQL_Byte src0[] = "abcdef";
