...
TEST_CASE("reql sindex status", "[reql][ast]") {
  ...
  SECTION("test11") {
    std::unique_ptr<ReQL_Obj_t> var0(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Byte[]> buf0(new ReQL_Byte[13]);
    const ReQL_Byte src0[] = "PTYPE<BINARY>";
    reql_string_init(var0.get(), buf0.get(), src0, 13);
  }
}
