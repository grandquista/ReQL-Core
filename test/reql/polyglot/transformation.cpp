...
TEST_CASE("reql Tests that manipulation data in tables 259", "[reql][ast]") {
  ...
  SECTION("test39") {
    std::unique_ptr<ReQL_Obj_t> var0(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Byte[]> buf0(new ReQL_Byte[16]);
    const ReQL_Byte src0[] = "SELECTION<ARRAY>";
    reql_string_init(var0.get(), buf0.get(), src0, 16);
  }
