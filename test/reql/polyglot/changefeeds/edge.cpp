...
TEST_CASE("reql Test edge cases of changefeed operations", "[reql][ast]") {
  ...
  SECTION("test36") {
    ...
    std::unique_ptr<ReQL_Obj_t> var2(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Byte[]> buf2(new ReQL_Byte[10]);
    const ReQL_Byte src2[] = "erroredres";
    reql_string_init(var2.get(), buf2.get(), src2, 10);
