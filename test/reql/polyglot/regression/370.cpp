...
TEST_CASE("reql Regression tests for issue 328", "[reql][ast]") {
  ...
  SECTION("test1") {
    ...
    std::unique_ptr<ReQL_Obj_t> var1(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Byte[]> buf1(new ReQL_Byte[9]);
    const ReQL_Byte src1[] = "rethinkdb";
    reql_string_init(var1.get(), buf1.get(), src1, 9);
