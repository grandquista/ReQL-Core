...
TEST_CASE("reql Test geo constructors", "[reql][ast]") {
  ...
  SECTION("test12") {
    ...
    reql_string_init(var12.get(), buf12.get(), src12, 10);
  ...
  SECTION("test15") {
    ...
    const ReQL_Byte src14[] = "type";
    reql_string_init(var14.get(), buf14.get(), src14, 4);

    std::unique_ptr<ReQL_Obj_t> var15(new ReQL_Obj_t);
    std::unique_ptr<ReQL_Byte[]> buf15(new ReQL_Byte[10]);
    const ReQL_Byte src15[] = "LineString";
    reql_string_init(var15.get(), buf15.get(), src15, 10);
